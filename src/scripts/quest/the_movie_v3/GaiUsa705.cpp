...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 2:
      {
        if( type != 2 ) Scene00003( player ); // Scene00003: Normal(Talk, NpcDespawn, TargetCanMove), id=WILRED
        break;
      }
      case 255:
      {
        if( type != 2 ) Scene00004( player ); // Scene00004: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=GUNDOBALD
        break;
      }
      default:
      {
