...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( actor == 1006951 || actorId == 1006951 ) // ACTOR0 = LARAINA
        {
...
private:
  ...
  {
    player.sendDebug( "GaiUsd016:66845 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=LARAINA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUsd016:66845 calling [BranchTrue]Scene00004: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=LARAINA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
