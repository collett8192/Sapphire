...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( actor == 1001978 || actorId == 1001978 ) // ACTOR1 = unknown
        {
        ...
        if( actor == 1004576 || actorId == 1004576 ) // ACTOR0 = unknown
        {
      ...
      case 2:
      {
        if( actor == 1006003 || actorId == 1006003 ) // ACTOR2 = unknown
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
...
private:
  ...
  {
    player.sendDebug( "SubWil160:66238 calling [BranchTrue]Scene00002: Normal(CutScene), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "SubWil160:66238 calling Scene00004: Normal(Talk, NpcDespawn, TargetCanMove), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      player.setQuestUI8AL( getId(), 1 );
