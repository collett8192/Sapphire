...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( actor == 1012331 || actorId == 1012331 ) // ACTOR1 = STABLEHAND01583
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
            Scene00002( player ); // Scene00002: Normal(Talk, YesNo, TargetCanMove), id=STABLEHAND01583
          }
        }
        if( actor == 1012332 || actorId == 1012332 ) // ACTOR2 = ARTOIREL
        {
      ...
      case 3:
      {
        Scene00007( player ); // Scene00007: Normal(Talk, NpcDespawn, TargetCanMove), id=ARTOIREL
        break;
      }
      case 255:
      {
        Scene00008( player ); // Scene00008: Normal(Talk, FadeIn, QuestReward, QuestComplete, TargetCanMove), id=REDWALD
        break;
      }
      default:
      {
...
private:
  ...
  {
    player.sendDebug( "HeaVna104:67119 calling [BranchTrue]Scene00001: Normal(Talk, FadeIn, QuestAccept, TargetCanMove), id=ARTOIREL" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "HeaVna104:67119 calling Scene00002: Normal(Talk, YesNo, TargetCanMove), id=STABLEHAND01583" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 != 50 )
      {
        ...
        player.forceZoneing( 397, 475, 225, 794, 3.1415 );
      }
    };
