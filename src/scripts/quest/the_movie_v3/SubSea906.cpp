...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        if( type != 2 ) Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, TargetCanMove), id=SWYGSKYF
        // +Callback Scene00001: Normal(QuestAccept), id=unknown
        break;
      }
      case 255:
      {
        if( type != 2 ) Scene00002( player ); // Scene00002: Normal(Talk, Message, QuestReward, QuestComplete, TargetCanMove), id=TIRORO
        break;
      }
      default:
      {
