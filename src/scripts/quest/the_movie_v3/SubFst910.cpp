...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( type != 2 ) Scene00002( player ); // Scene00002: Normal(SystemTalk), id=unknown
        break;
      }
      case 255:
      {
        if( type != 2 ) Scene00003( player ); // Scene00003: Normal(FadeIn, TargetCanMove, SystemTalk), id=unknown
