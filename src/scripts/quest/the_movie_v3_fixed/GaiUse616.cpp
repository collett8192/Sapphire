...
//fix: auto entering the room so ARR does not end up with a black screen
  ...
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        ...
        if( actor == 1010928 || actorId == 1010928 ) // ACTOR1 = ALPHINAUD
        {
      ...
      case 255:
      {
        if( actor == 1007603 || actorId == 1007603 ) // ACTOR2 = FORTEMPSGUARD00428
        {
          Scene00004( player ); // Scene00004: Normal(Talk, YesNo, QuestReward, QuestComplete, TargetCanMove), id=FORTEMPSGUARD00428
...
private:
  ...
  {
    player.sendDebug( "GaiUse616:65964 calling [BranchTrue]Scene00001: Normal(CutScene), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUse616:65964 calling [BranchFalse]Scene00002: Normal(QuestAccept), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
    };
  ...
  {
    player.sendDebug( "GaiUse616:65964 calling Scene00004: Normal(Talk, YesNo, QuestReward, QuestComplete, TargetCanMove), id=FORTEMPSGUARD00428" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
  ...
  {
    player.sendDebug( "GaiUse616:65964 calling [BranchTrue]Scene00005: Normal(CutScene), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
