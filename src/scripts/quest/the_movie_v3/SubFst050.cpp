...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        Scene00001( player ); // Scene00001: NpcTrade(Talk, TargetCanMove), id=KEITHA
...
private:
  ...
  {
    player.sendDebug( "SubFst050:65692 calling [BranchTrue]Scene00100: Normal(QuestReward, QuestComplete), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
  ...
  {
    player.sendDebug( "SubFst050:65692 calling Scene00099: Normal(None), id=unknown" );
  }
};
