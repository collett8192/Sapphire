...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        if( actor == 1011020 || actorId == 1011020 ) // ACTOR0 = TATARU
        {
          Scene00000( player ); // Scene00000: Normal(QuestOffer, TargetCanMove), id=unknown
          // +Callback Scene00001: Normal(Talk, NpcDespawn, QuestAccept, TargetCanMove), id=TATARU
        }
        if( actor == 1000909 || actorId == 1000909 ) // ACTOR1 = THUBYRGEIM
        {
      ...
      case 1:
      {
        Scene00003( player ); // Scene00003: Normal(Talk, NpcDespawn, TargetCanMove), id=TATARU
        break;
      }
      case 2:
      {
      ...
      case 3:
      {
        if( actor == 2005064 || actorId == 2005064 ) // EOBJECT0 = unknown
        {
        ...
        if( actor == 5584488 || actorId == 5584488 ) // ENEMY0 = unknown
        {
          Scene00007( player ); // Scene00007: Normal(None), id=unknown
          // +Callback Scene00008: Normal(Message, PopBNpc), id=unknown
        }
        if( actor == 1010983 || actorId == 1010983 ) // ACTOR3 = TATARU_2
        {
      ...
      case 4:
      {
        Scene00010( player ); // Scene00010: Normal(Talk, FadeIn, TargetCanMove), id=TATARU
        break;
      }
      case 255:
      {
        Scene00011( player ); // Scene00011: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=MINFILIA
        break;
      }
      default:
      {
...
private:
  ...
  {
    player.sendDebug( "GaiUse610:65958 calling [BranchTrue]Scene00001: Normal(Talk, NpcDespawn, QuestAccept, TargetCanMove), id=TATARU" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUse610:65958 calling Scene00005: Normal(None), id=unknown" );
    Scene00006( player );
  }
  void Scene00006( Entity::Player& player )
  {
    player.sendDebug( "GaiUse610:65958 calling [BranchTrue]Scene00006: Normal(Message, PopBNpc), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUse610:65958 calling Scene00007: Normal(None), id=unknown" );
    Scene00008( player );
  }
  void Scene00008( Entity::Player& player )
  {
    player.sendDebug( "GaiUse610:65958 calling [BranchTrue]Scene00008: Normal(Message, PopBNpc), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      checkProgressSeq3( player );
    };
  ...
  {
    player.sendDebug( "GaiUse610:65958 calling Scene00011: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=MINFILIA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
