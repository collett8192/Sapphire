...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        if( actor == 1006573 || actorId == 1006573 ) // ACTOR0 = MINFILIA
        {
      ...
      case 255:
      {
        ...
        // +Callback Scene00006: Normal(QuestReward, QuestComplete), id=unknown
        break;
      }
      default:
      {
...
private:
  ...
  {
    player.sendDebug( "ManFst408:66058 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=MINFILIA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "ManFst408:66058 calling [BranchTrue]Scene00005: Normal(CutScene), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      Scene00006( player );
    };
  ...
  {
    player.sendDebug( "ManFst408:66058 calling [BranchChain]Scene00006: Normal(QuestReward, QuestComplete), id=unknown" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
