  ...
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 255:
      {
        ...
        if( actor == 1009077 || actorId == 1009077 ) // ACTOR3 = HONBUBOUKENSHA01354
        {
          Scene00006( player ); // Scene00006: Normal(Talk, TargetCanMove), id=HONBUBOUKENSHA01354
        }
        if( actor == 1009078 || actorId == 1009078 ) // ACTOR4 = unknown
        {
          Scene00007( player ); // Scene00007: Normal(None), id=unknown
        }
        if( actor == 1009079 || actorId == 1009079 ) // ACTOR5 = unknown
        {
          Scene00008( player ); // Scene00008: Normal(None), id=unknown
        }
        if( actor == 1009080 || actorId == 1009080 ) // ACTOR6 = unknown
        {
...
private:
  ...
  {
    player.sendDebug( "GaiUse317:66994 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=MINFILIA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
