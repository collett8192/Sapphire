...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        if( type != 2 ) Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=ZEZEDA
        break;
      }
      ...
      case 255:
      {
        if( type != 2 ) Scene00001( player ); // Scene00001: NpcTrade(Talk, TargetCanMove), id=BERINGAER
...
private:
  ...
  {
    player.sendDebug( "SubWil072:65862 calling Scene00001: NpcTrade(Talk, TargetCanMove), id=BERINGAER" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
  ...
  {
    player.sendDebug( "SubWil072:65862 calling Scene00100: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=BERINGAER" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
