  ...
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        ...
        // +Callback Scene00001: Normal(Talk, NpcDespawn, QuestAccept, TargetCanMove), id=MERLWYB
        break;
      }
      case 1:
      {
        if( actor == 1002694 || actorId == 1002694 ) // ACTOR1 = unknown
        {
      ...
      case 2:
      {
        if( actor == 1008679 || actorId == 1008679 ) // ACTOR6 = YUGIRI
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
            Scene00009( player ); // Scene00009: Normal(Talk, FadeIn, TargetCanMove), id=YUGIRI
          }
        }
        if( actor == 1008676 || actorId == 1008676 ) // ACTOR3 = MINFILIA
        {
      ...
      case 3:
      {
        if( actor == 1008692 || actorId == 1008692 ) // ACTOR8 = STORMPRIVATE01360
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
            Scene00015( player ); // Scene00015: Normal(Talk, FadeIn, TargetCanMove), id=STORMPRIVATE01360
          }
        }
        if( actor == 1002694 || actorId == 1002694 ) // ACTOR1 = MERLWYB
        {
...
private:
  ...
  {
    player.sendDebug( "GaiUse216:66896 calling [BranchTrue]Scene00001: Normal(Talk, NpcDespawn, QuestAccept, TargetCanMove), id=MERLWYB" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUse216:66896 calling Scene00009: Normal(Talk, FadeIn, TargetCanMove), id=YUGIRI" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      player.setQuestUI8AL( getId(), 1 );
  ...
  {
    player.sendDebug( "GaiUse216:66896 calling Scene00015: Normal(Talk, FadeIn, TargetCanMove), id=STORMPRIVATE01360" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      player.setQuestUI8AL( getId(), 1 );
