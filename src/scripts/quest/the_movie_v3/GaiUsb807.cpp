...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( actor == 4334543 || actorId == 4334543 ) // EVENTRANGE0 = unknown
        {
        ...
        if( actor == 2002152 || actorId == 2002152 ) // EOBJECT0 = unknown
        {
          Scene00003( player ); // Scene00003: Normal(Message), id=unknown
          // +Callback Scene00004: Normal(None), id=unknown
        }
        if( actor == 4293186 || actorId == 4293186 ) // ENEMY0 = unknown
        {
          // empty entry
        }
        if( actor == 2002512 || actorId == 2002512 ) // EOBJECT1 = unknown
        {
...
private:
  ...
  {
    player.sendDebug( "GaiUsb807:66452 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=EUGENNOIX" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUsb807:66452 calling [BranchTrue]Scene00004: Normal(None), id=unknown" );
    checkProgressSeq1( player );
  }

  void Scene00005( Entity::Player& player )
  {
    player.sendDebug( "GaiUsb807:66452 calling Scene00005: Normal(None), id=unknown" );
    Scene00006( player );
  }
  void Scene00006( Entity::Player& player )
  {
    player.sendDebug( "GaiUsb807:66452 calling [BranchTrue]Scene00006: Normal(None), id=unknown" );
    checkProgressSeq1( player );
  }
  ...
  {
    player.sendDebug( "GaiUsb807:66452 calling [BranchTrue]Scene00008: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=EUGENNOIX" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
