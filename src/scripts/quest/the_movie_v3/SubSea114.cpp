...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=MIMIDOA
        break;
      }
...
private:
  ...
  {
    player.sendDebug( "SubSea114:65947 calling [BranchTrue]Scene00003: Normal(Talk, TargetCanMove), id=ROSTNZEH" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
