...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( type != 2 ) Scene00002( player ); // Scene00002: Normal(Talk, FadeIn, TargetCanMove), id=DELLEXIA
        break;
      }
      case 2:
      {
      ...
      case 255:
      {
        if( type != 2 ) Scene00007( player ); // Scene00007: Normal(Talk, NpcDespawn, QuestReward, QuestComplete, TargetCanMove), id=URIANGER
        break;
      }
      default:
      {
...
private:
  ...
  void Scene00006( Entity::Player& player ) //SEQ_3: ACTOR1, <No Var>, <No Flag>
  {
