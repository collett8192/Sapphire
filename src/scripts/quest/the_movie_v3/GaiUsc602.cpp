...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        if( actor == 4324033 || actorId == 4324033 ) // EVENTRANGE0 = unknown
        {
        ...
        if( actor == 2002238 || actorId == 2002238 ) // EOBJECT0 = unknown
        {
          if( player.getQuestUI8AL( getId() ) != 2 )
          {
        ...
        if( actor == 4300144 || actorId == 4300144 ) // ENEMY0 = unknown
        {
          // empty entry
        }
        if( actor == 4300145 || actorId == 4300145 ) // ENEMY1 = unknown
        {
          // empty entry
        }
        if( actor == 2002546 || actorId == 2002546 ) // EOBJECT1 = unknown
        {
      ...
      case 255:
      {
        ...
        // +Callback Scene00007: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=GLAUMUNT
        break;
      }
      default:
      {
...
private:
  ...
  {
    player.sendDebug( "GaiUsc602:66538 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=GLAUMUNT" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
  ...
  {
    player.sendDebug( "GaiUsc602:66538 calling [BranchTrue]Scene00005: Normal(None), id=unknown" );
    checkProgressSeq1( player );
  }

  void Scene00006( Entity::Player& player )
  {
    player.sendDebug( "GaiUsc602:66538 calling Scene00006: Normal(None), id=unknown" );
    Scene00007( player );
  }
  void Scene00007( Entity::Player& player )
  {
    player.sendDebug( "GaiUsc602:66538 calling [BranchTrue]Scene00007: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=GLAUMUNT" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
