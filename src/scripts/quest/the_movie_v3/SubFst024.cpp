...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        if( type != 2 ) Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=EMERIA
        break;
      }
      //seq 1 event item ITEM0 = UI8BH max stack 1
      case 1:
      {
        if( param1 == 2000009 || param2 == 2000009 ) // EOBJECT0 = unknown
        {
        ...
        if( param1 == 2313265 || param2 == 2313265 ) // ENEMY0 = unknown
        {
      ...
      case 255:
      {
        if( type != 2 ) Scene00007( player ); // Scene00007: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=MIOUNNE
        break;
      }
      default:
      {
...
private:
  ...
  {
    player.sendDebug( "SubFst024:65635 calling Scene00005: Normal(Talk, FadeIn, TargetCanMove), id=EMERIA" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
