...
class SubSea005 : public Sapphire::ScriptAPI::EventScript
{
public:
  SubSea005() : Sapphire::ScriptAPI::EventScript( 65651 ){}; 
  ~SubSea005() = default; 
...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 0:
      {
        if( type != 2 ) Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=PTAHJHA
        break;
      }
      //seq 1 event item ITEM0 = UI8BH max stack 1
      case 1:
      {
        if( type != 2 ) Scene00002( player ); // Scene00002: NpcTrade(Talk, TargetCanMove), id=SUNDHIMAL
      ...
      case 255:
      {
        if( type != 2 ) Scene00004( player ); // Scene00004: NpcTrade(Talk, TargetCanMove), id=LATISHA
