...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 0:
      {
        if( type != 2 ) Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=SWOZBLAET
        break;
      }
      //seq 1 event item ITEM0 = UI8CH max stack 1
      case 1:
      {
        if( param1 == 1001203 || param2 == 1001203 ) // ACTOR1 = IRONTHUNDER
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
            Scene00002( player ); // Scene00002: Normal(Talk, TargetCanMove), id=IRONTHUNDER
          }
          break;
        }
        if( param1 == 1001208 || param2 == 1001208 ) // ACTOR2 = KYOKYOROON
        {
        ...
        if( param1 == 1003254 || param2 == 1003254 ) // ACTOR3 = SYNEYHIL
        {
      ...
      case 255:
      {
        if( type != 2 ) Scene00005( player ); // Scene00005: NpcTrade(Talk, TargetCanMove), id=BAENSYNG
