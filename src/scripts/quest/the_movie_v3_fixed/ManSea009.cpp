...
private:
  void onProgress( Entity::Player& player, uint64_t param1, uint32_t param2, uint32_t type, uint32_t param3 )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=BADERON
        break;
      }
      case 1:
      {
        if( param1 == 1001029 || param2 == 1001029 ) // ACTOR1 = ZANTHAEL
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
            Scene00002( player ); // Scene00002: Normal(Talk, TargetCanMove), id=ZANTHAEL
            // +Callback Scene00003: Normal(Talk, TargetCanMove), id=ZANTHAEL
            // +Callback Scene00004: Normal(CutScene, AutoFadeIn), id=unknown
          }
          break;
        }
        if( param1 == 1003355 || param2 == 1003355 ) // ACTOR2 = JNASSHYM
        {
          Scene00006( player ); // Scene00006: Normal(Talk, TargetCanMove), id=JNASSHYM
          break;
        }
        break;
      }
      case 255:
      {
        if( param1 == 1002694 || param2 == 1002694 ) // ACTOR3 = MERLWYB
        {
          Scene00007( player ); // Scene00007: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=MERLWYB
          break;
        }
        if( param1 == 1001029 || param2 == 1001029 ) // ACTOR1 = ZANTHAEL
        {
          Scene00008( player ); // Scene00008: Normal(Talk, YesNo, TargetCanMove, CanCancel), id=ZANTHAEL
          break;
        }
        break;
      }
