  ...
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      ...
      case 1:
      {
        ...
        if( actor == 1009996 || actorId == 1009996 ) // ACTOR2 = ALPHINAUD
        {
          Scene00003( player ); // Scene00003: Normal(Talk, TargetCanMove), id=ALPHINAUD
        }
        if( actor == 1010002 || actorId == 1010002 ) // ACTOR3 = LUCIA
        {
        ...
        if( actor == 1009997 || actorId == 1009997 ) // ACTOR4 = YUYUHASE
        {
          Scene00006( player ); // Scene00006: Normal(Talk, TargetCanMove), id=YUYUHASE
        }
        if( actor == 1010054 || actorId == 1010054 ) // ACTOR5 = TEMPLER00075
        {
      ...
      case 2:
      {
        if( actor == 1009996 || actorId == 1009996 ) // ACTOR2 = unknown
        {
          if( player.getQuestUI8AL( getId() ) != 1 )
          {
...
private:
  ...
  {
    player.sendDebug( "GaiUse408:65611 calling [BranchTrue]Scene00001: Normal(Talk, QuestAccept, TargetCanMove), id=DRILLEMONT" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
