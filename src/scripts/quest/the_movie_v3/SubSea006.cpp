...
class SubSea006 : public Sapphire::ScriptAPI::EventScript
{
public:
  SubSea006() : Sapphire::ScriptAPI::EventScript( 65652 ){}; 
  ~SubSea006() = default; 
...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=HLAHONO
        break;
      }
      case 1:
      {
      ...
      case 2:
      {
        Scene00002( player ); // Scene00002: Normal(Talk, TargetCanMove), id=ABYLFARR
        break;
      }
...
private:
  ...
  {
    player.sendDebug( "SubSea006:65652 calling Scene00001: Normal(None), id=unknown" );
  ...
  {
    player.sendDebug( "SubSea006:65652 calling [BranchTrue]Scene00004: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=HLAHONO" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
