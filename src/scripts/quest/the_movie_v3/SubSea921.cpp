...
class SubSea921 : public Sapphire::ScriptAPI::EventScript
{
public:
  SubSea921() : Sapphire::ScriptAPI::EventScript( 66235 ){}; 
  ~SubSea921() = default; 
...
private:
  void onProgress( Entity::Player& player, uint64_t actorId, uint32_t actor, uint32_t type, uint32_t param )
  {
    switch( player.getQuestSeq( getId() ) )
    {
      case 0:
      {
        Scene00000( player ); // Scene00000: Normal(Talk, QuestOffer, QuestAccept, TargetCanMove), id=SWYRGEIM
        break;
      }
      case 255:
      {
...
private:
  ...
  {
    player.sendDebug( "SubSea921:66235 calling [BranchTrue]Scene00002: Normal(Talk, QuestReward, QuestComplete, TargetCanMove), id=SWYRGEIM" );
    auto callback = [ & ]( Entity::Player& player, const Event::SceneResult& result )
    {
      if( result.param1 > 0 && result.param2 == 1 )
      {
